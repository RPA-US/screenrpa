...
from core.settings import (
    SINGLE_FE_EXTRACTORS_FILEPATH,
    AGGREGATE_FE_EXTRACTORS_FILEPATH,
    POSTPROCESSING_TECHNIQUES_FILEPATH,
)
from .models import (
...
def get_ui_elements_classification(case_study):
    return get_object_or_404(
        UIElementsClassification, case_study=case_study, active=True
    )
...
def get_feature_extraction_technique(case_study):
    return get_object_or_404(
        FeatureExtractionTechnique, case_study=case_study, active=True
    )
...
def execution_has_feature_extraction_technique(execution, type="ANY"):
    if type == "SINGLE":
        res = (
            execution.feature_extraction_technique
            and execution.feature_extraction_technique.type == "SINGLE"
        )
    elif type == "AGGREGATE":
        res = (
            execution.feature_extraction_technique
            and execution.feature_extraction_technique.type == "AGGREGATE"
        )
    else:
...
def save_corners_json(
    file_path, compos, img_index, texto_detectado_ocr, text_classname, applied_ocr
):
    ...
    print(
        "Number of text boxes detected (iteration "
        + str(img_index)
        + "): "
        + str(len(texto_detectado_ocr[img_index]) if applied_ocr else 0)
    )
    ...
    for x in range(0, row, step_h):
        for y in range(0, column, step_v):
            if mask[x, y] == 0:
                ...
                if (
                    compo.area / (row * column)
                    > uied_params["nested-shared-area-percentage"]
                ):
                    continue
                elif (
                    compo.area / (row * column)
                    > uied_params["nested-shared-area-to-be-redundant"]
                ):
                ...
                if uied_params["nested-ignore-non-rectangle-blocks"] or (
                    not compo.compo_is_rectangle(min_rec_evenness, max_dent_ratio)
                ):
                    continue
    ...
    for i in range(0, row, uied_params["step_h"]):
        for j in range(i % 2, column, uied_params["step_w"]):
            if binary[i, j] == 255 and mask[i, j] == 0:
                ...
                # ignore small area
                if (
                    component.width <= uied_params["compo-width-considered-small"]
                    or component.height <= uied_params["compo-height-considered-small"]
                ):
                    continue
...
def is_block(clip, thread=0.15):
    ...
    if map(lambda c: c < 9, clip.shape):
        return False
...
def draw_ui_compos_borders(exp_path):
    ...
    for compo_json_filename in arr:
        ...
        for compo in compo_json["compos"]:
            ...
            # Escalar coordenadas
            x_min, y_min = scale_coordinates(
                (x_min, y_min), src_resolution, dest_resolution
            )
            x_max, y_max = scale_coordinates(
                (x_max, y_max), src_resolution, dest_resolution
            )
...
def polygon_list_to_draw(
    polygons, draw: ImageDraw, fill=(255, 0, 0, 128), outline=(255, 0, 0, 255)
):
    if isinstance(polygons, (MultiPolygon, GeometryCollection)):
...
def draw_geometry_over_image(
    background_image_path, circles, rectangles, output_image_path
):
    ...
    result_image = Image.alpha_composite(
        background_image.convert("RGBA"), overlay_image
    )
...
def get_compo_from_xpath(
    path_scenario: str, img_name: str, xpath: list[str]
) -> dict | None:
    """
    ...
    match len(matching_compos):
        ...
        case _:
            raise Exception(
                f"More than one component with the same xpath in img {img_name}, scenario {path_scenario}, xpath {xpath}"
            )
