...
class UIElementsDetectionForm(forms.ModelForm):
    class Meta:
        ...
        labels = {"type": _("Type"), "skip": _("Skip")}
        ...
        widgets = {
            "title": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "UI Elements Detection"}
            ),
            ...
            "ocr": forms.CheckboxInput(attrs={"class": "primary-checkbox"}),
...
class PostfiltersForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "consider_nested_as_relevant": forms.CheckboxInput(
    ...
    class Meta:
        ...
        widgets = {
            "type": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "uied"}
            ),
        }
...
class FeatureExtractionTechniqueForm(forms.ModelForm):
    class Meta:
        ...
        labels = {
            ...
            "consider_relevant_compos": _(
                "Apply Filtering (Relevant Component Selection)"
            ),
            "relevant_compos_predicate": _(
                "Condition for a UI Component to be relevant"
            ),
            "preloaded_file": "Preload Execution Results",
            "title": "Title ",
        }
        ...
        widgets = {
            ...
            "identifier": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "sta_s"}
            ),
            "preloaded_file": forms.FileInput(attrs={"accept": ".zip"}),
            "preloaded": forms.CheckboxInput(attrs={"class": "primary-checkbox"}),
            "type": forms.Select(
                choices=[
                    ...
                    (
                        "SINGLE",
                        "Single: Feature extraction just after UI Elem. Detection",
                    ),
                    (
                        "AGGREGATE",
                        "Aggregate: Feature extraction after flattening UI log into dataset",
                    ),
                ],
                attrs={
                    "class": "form-control",
...
class PostprocessingForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "title": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Postprocessing"}
            ),
            "preloaded_file": forms.FileInput(attrs={"accept": ".zip"}),
            "preloaded": forms.CheckboxInput(attrs={"class": "primary-checkbox"}),
            "technique_name": forms.Select(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["technique_name"].widget.choices = list(
            map(lambda x: list(x), techniques_json.items())
        )
        if self.read_only:
            for field_name in self.fields:
                self.fields[field_name].disabled = True
