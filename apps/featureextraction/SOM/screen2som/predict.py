...
def predict(image_path, img_index, path_to_save_bordered_images, text_detected_by_OCR):
    ...
    for model_name, properties in models.items():
        id_start = (
            0 if len(detections["compos"]) == 0 else len(detections["compos"]) + 1
        )
        if properties["function"] == "yolo":
            compos = yolo_prediction(
                os.path.join(json_config["models_path"], model_name + ".pt"),
                image_pil_resized,
                properties["type"],
                id_start,
            )
    ...
    for compo in detections["compos"]:
        for point in compo["points"]:
            ...
            point[0] = (
                point[0] * detections["img_shape"][1] / image_pil_resized.shape[1]
            )
            point[1] = (
                point[1] * detections["img_shape"][0] / image_pil_resized.shape[0]
            )
    ...
    include_OCR_as_text(detections, img_index, text_detected_by_OCR)

    # SOM from shapes
    predictions = labels_to_output(copy.deepcopy(detections))

    # Save bordered images
    save_bordered_images(
        image_path,
        detections["compos"],
...
def yolo_prediction(model_path, image_pil, type, id_start):
    ...
    shapes = json_inference_to_compos(result, type=type, id_start=id_start)
...
def sahi_predictions(
    ...
    shapes = coco_to_compos(anns, type=type, id_start=id_start)
