...
from core.settings import (
    sep,
    PLATFORM_NAME,
    CLASSIFICATION_PHASE_NAME,
    SINGLE_FEATURE_EXTRACTION_PHASE_NAME,
    AGGREGATE_FEATURE_EXTRACTION_PHASE_NAME,
)
...
from apps.featureextraction.SOM.classification import (
    legacy_ui_elements_classification,
    uied_ui_elements_classification,
)
from .models import (
...
from .forms import (
    UIElementsClassificationForm,
    UIElementsDetectionForm,
    PrefiltersForm,
    PostfiltersForm,
    FeatureExtractionTechniqueForm,
    PostprocessingForm,
)
from .relevantinfoselection.postfilters import (
    draw_postfilter_relevant_ui_compos_borders,
)
from .utils import (
    detect_single_fe_function,
    detect_agg_fe_function,
    detect_postprocessing_function,
    read_ui_log_as_dataframe,
    draw_ui_compos_borders,
)
...
def ui_elements_classification(log_path, path_scenario, execution):
    ...
    if not execution.ui_elements_detection.preloaded:
        match str.lower(execution.ui_elements_classification.type):
            case "rpa-us":
                output = legacy_ui_elements_classification(
                    log_path, path_scenario, execution
                )
            case "uied":
                output = uied_ui_elements_classification(
                    log_path, path_scenario, execution
                )
            case "sam":
                output = legacy_ui_elements_classification(
            ...
            case _:
                raise Exception(
                    "You select a type of UI element classification that doesnt exists"
                )
    else:
        output = None
    return output
...
def feature_extraction_technique(log_path, path_scenario, execution, fe):
    ...
    if not skip:
        if fe_type == "SINGLE":
            tprint(
                PLATFORM_NAME + " - " + SINGLE_FEATURE_EXTRACTION_PHASE_NAME, "fancy60"
            )
            print(
                "Single feature extraction selected: "
                + feature_extraction_technique_name
                + "\n"
            )
            output = detect_single_fe_function(feature_extraction_technique_name)(
                log_path, path_scenario, execution, fe
            )
        else:
            tprint(
                PLATFORM_NAME + " - " + AGGREGATE_FEATURE_EXTRACTION_PHASE_NAME,
                "fancy60",
            )
            print(
                "Aggregate feature extraction selected: "
                + feature_extraction_technique_name
                + "\n"
            )
            output = detect_agg_fe_function(feature_extraction_technique_name)(
                log_path, path_scenario, execution, fe
            )
    return output
...
def postprocessing(log_path, path_scenario, execution, pp):
    ...
    detect_postprocessing_function(pp.technique_name)(
        log_path, path_scenario, execution, pp.gaze_conciliation
    )
    return None
...
class FeatureExtractionTechniqueCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(FeatureExtractionTechniqueCreateView, self).get_context_data(
            **kwargs
        )
        ...
        aggregate_json = json.load(
            open("configuration/aggregate_feature_extractors.json")
        )
        context["options"] = {
...
class FeatureExtractionTechniqueListView(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not case_study:
            ...
            raise PermissionDenied(
                "Case Study doesn't belong to the authenticated user."
            )
        if "FeatureExtractionTechnique" in case_study.available_phases:
            return super().get(request, *args, **kwargs)
        else:
    ...
    def get_context_data(self, **kwargs):
        context = super(FeatureExtractionTechniqueListView, self).get_context_data(
            **kwargs
        )
        context["case_study_id"] = self.kwargs.get("case_study_id")
        return context
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = FeatureExtractionTechnique.objects.filter(
...
class FeatureExtractionTechniqueDetailView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=...) -> Model:
        return get_object_or_404(
            FeatureExtractionTechnique,
            id=self.kwargs["feature_extraction_technique_id"],
        )
    ...
    def get(self, request, *args, **kwargs):
        feature_extraction = get_object_or_404(
            FeatureExtractionTechnique, id=kwargs["feature_extraction_technique_id"]
        )
        ...
        form = FeatureExtractionTechniqueForm(
            read_only=feature_extraction.freeze, instance=feature_extraction
        )
        ...
        if "case_study_id" in kwargs:
            ...
            if "FeatureExtractionTechnique" in case_study.available_phases:
                context = context | {
                    "feature_extraction_technique": feature_extraction,
                    "case_study_id": case_study.id,
                    "form": form,
                }

                return render(
                    request, "feature_extraction_technique/detail.html", context
                )
            else:
                return HttpResponseRedirect(reverse("analyzer:casestudy_list"))
...
def set_as_feature_extraction_technique_active(request):
    ...
    feature_extraction_technique = FeatureExtractionTechnique.objects.get(
        id=feature_extraction_technique_id
    )
    feature_extraction_technique.active = True
    feature_extraction_technique.save()
    return HttpResponseRedirect(
        reverse("featureextraction:fe_technique_list", args=[case_study_id])
    )
...
def set_as_feature_extraction_technique_inactive(request):
    ...
    if (
        FeatureExtractionTechnique.objects.get(pk=feature_extraction_technique_id).user
        != request.user
    ):
        raise ValidationError(
            _("Feature Extraction Technique doesn't belong to the authenticated user.")
        )
    if FeatureExtractionTechnique.objects.get(
        pk=feature_extraction_technique_id
    ).case_study != CaseStudy.objects.get(pk=case_study_id):
        raise ValidationError(
            _(
                "Feature Extraction Technique Tree Training doesn't belong to the Case Study."
            )
        )
    feature_extraction_technique = FeatureExtractionTechnique.objects.get(
        id=feature_extraction_technique_id
...
class PostprocessingCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Load single and aggregate techniques from configurations
        techniques_json = json.load(
            open("configuration/postprocessing_techniques.json")
        )
        context["options"] = {
            # We convert the tuples returned by the items() method to lists so that javascript can correctly parse them
...
class PostprocessingListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Filtra los objetos por case_study_id
        if search:
            queryset = Postprocessing.objects.filter(
                case_study__id=case_study_id,
                case_study__user=self.request.user,
                title__icontains=search,
            ).order_by("-created_at")
...
class PostprocessingDetailView(LoginRequiredMixin, UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        postprocessing = get_object_or_404(
            Postprocessing, id=kwargs["postprocessing_id"]
        )
        if not postprocessing:
            return HttpResponse(status=404, content="FE not found.")
        elif postprocessing.case_study.user != request.user:
            raise PermissionDenied(
                "Postprocessing doesn't belong to the authenticated user."
            )

        form = PostprocessingForm(
            read_only=postprocessing.freeze, instance=postprocessing
        )
        if "case_study_id" in kwargs:
            case_study = get_object_or_404(CaseStudy, id=kwargs["case_study_id"])
            if "Postprocessing" in case_study.available_phases:
...
def set_as_postprocessing_active(request):
    ...
    postprocessing.save()
    return HttpResponseRedirect(
        reverse("featureextraction:postprocessing_list", args=[case_study_id])
    )
...
def set_as_postprocessing_inactive(request):
    ...
    if Postprocessing.objects.get(pk=postprocessing_id).user != request.user:
        raise ValidationError(
            _("Postprocessing doesn't belong to the authenticated user.")
        )
    if Postprocessing.objects.get(
        pk=postprocessing_id
    ).case_study != CaseStudy.objects.get(pk=case_study_id):
...
class UIElementsDetectionCreateView(LoginRequiredMixin, MultiFormsView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not case_study:
            ...
            raise PermissionDenied(
                "UI Elm Det doesn't belong to the authenticated user."
            )
    ...
    def forms_valid(self, forms):
        ...
        ui_elem_det_obj = self.ui_elements_detection_form_valid(
            ui_elements_detection_form
        )
        self.ui_elements_classification_form_valid(
            ui_elements_classification_form, ui_elem_det_obj
        )
...
class UIElementsDetectionListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Filtra los objetos por case_study_id
        if search:
            queryset = UIElementsDetection.objects.filter(
                case_study__id=case_study_id,
                case_study__user=self.request.user,
                title__icontains=search,
            ).order_by("-created_at")
...
class UIElementsDetectionDetailView(LoginRequiredMixin, MultiFormsView):
    ...
    def get_ui_elements_detection_initial(self):
        ui_elements_detection = get_object_or_404(
            UIElementsDetection, id=self.kwargs["ui_elements_detection_id"]
        )
        # Return serialized
        return {
            "initial": {
    ...
    def get_ui_elements_classification_initial(self):
        ...
        if ui_elements_detection.ui_elements_classification:
            ui_elements_classification = (
                ui_elements_detection.ui_elements_classification
            )
        else:
    ...
    def ui_elements_detection_form_valid(self, form):
        if not self.request.user.is_authenticated:
            raise ValidationError("User must be authenticated.")
        self.object = UIElementsDetection.objects.filter(
            pk=self.kwargs["ui_elements_detection_id"], user=self.request.user
        )
        if self.object.exists():
            self.object.update(**form.cleaned_data)
        else:
            raise ValidationError(
                "This object doesn't belong to the authenticated user"
            )
    ...
    def ui_elements_classification_form_valid(self, form, ui_elem_det_obj):
        ...
        if not ui_elem_det_obj.ui_elements_classification:
            ...
            UIElementsClassification.objects.filter(
                pk=ui_elem_det_obj.ui_elements_classification.id
            ).delete()
        else:
            UIElementsClassification.objects.filter(
                pk=ui_elem_det_obj.ui_elements_classification.id
...
def set_as_ui_elements_detection_active(request):
    ...
    if (
        UIElementsDetection.objects.get(pk=ui_elements_detection_id).user
        != request.user
    ):
        raise PermissionDenied(
            "UI Element Detection doesn't belong to the authenticated user."
        )
    if UIElementsDetection.objects.get(
        pk=ui_elements_detection_id
    ).case_study != CaseStudy.objects.get(pk=case_study_id):
    ...
    UIElementsClassification.objects.filter(
        case_study_id=case_study_id, active=True
    ).update(active=False)
    if ui_elements_detection.ui_elements_classification:
    ...
    return HttpResponseRedirect(
        reverse("featureextraction:ui_detection_list", args=[case_study_id])
    )
...
class PrefiltersCreateView(LoginRequiredMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not case_study:
            ...
            raise PermissionDenied(
                "Prefilter doesn't belong to the authenticated user."
            )
...
class PrefiltersListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Filtra los objetos por case_study_id
        if search:
            queryset = Prefilters.objects.filter(
                case_study__id=case_study_id,
                case_study__user=self.request.user,
                title__icontains=search,
            ).order_by("-created_at")
...
def set_as_prefilters_active(request):
    ...
    Postfilters.objects.filter(case_study_id=case_study_id, active=True).update(
        active=False
    )
    return HttpResponseRedirect(
        reverse("featureextraction:prefilters_list", args=[case_study_id])
    )
...
class PostfiltersCreateView(LoginRequiredMixin, CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not case_study:
            ...
            raise PermissionDenied(
                "Postfilters doesn't belong to the authenticated user."
            )
...
class PostfiltersListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Filtra los objetos por case_study_id
        if search:
            queryset = Postfilters.objects.filter(
                case_study__id=case_study_id,
                case_study__user=self.request.user,
                title__icontains=search,
            ).order_by("-created_at")
...
def set_as_postfilters_active(request):
    ...
    Prefilters.objects.filter(case_study_id=case_study_id, active=True).update(
        active=False
    )
    return HttpResponseRedirect(
        reverse("featureextraction:postfilters_list", args=[case_study_id])
    )
...
def set_as_postfilters_inactive(request):
    ...
    if Postfilters.objects.get(pk=postfilter_id).user != request.user:
        raise ValidationError(
            _("Postfiltering doesn't belong to the authenticated user.")
        )
    if Postfilters.objects.get(pk=postfilter_id).case_study != CaseStudy.objects.get(
        pk=case_study_id
    ):
    ...
    try:
        ...
        if execution.executed:
            ...
            for scenario in execution.scenarios_to_study:
                draw_postfilter_relevant_ui_compos_borders(
                    os.path.join(execution.exp_folder_complete_path, scenario)
                )
...
class UIElementsDetectionResultDetailView(LoginRequiredMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.id != execution.user.id:
            raise PermissionDenied(
                "Execution doesn't belong to the authenticated user."
            )
        ...
        for compo_json in os.listdir(
            ...
            img_path = os.path.join(
                execution.case_study.exp_foldername, scenario, img_name
            )

            soms["soms"].append({"img": img_name, "img_path": img_path})
...
class PrefilteringResultDetailView(LoginRequiredMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for img in os.listdir(
            ...
            # path is something like: asdsa/.../.../image.PNG.json
            img_path = os.path.join(
                execution.case_study.exp_foldername,
                "executions",
            ...
            img_list.append({"img": img, "img_path": img_path})
