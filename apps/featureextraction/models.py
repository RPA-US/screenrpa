...
# Gaze conciliation options used in postprocessing configurations
GAZE_CONCILIATION_CHOICES = (
    ("union", "Union"),
...
class Prefilters(models.Model):
    ...
    case_study = models.ForeignKey(
        "apps_analyzer.CaseStudy", on_delete=models.CASCADE, null=True
    )
    user = models.ForeignKey(User, on_delete=models.CASCADE)

    def get_absolute_url(self):
        return reverse(
            "featureextraction:prefilters_list", args=[str(self.case_study_id)]
        )
...
class UIElementsDetection(models.Model):
    ...
    type = models.CharField(
        max_length=25, choices=UI_ELM_DET_TYPES, default="rpa-us", blank=True, null=True
    )
    ...
    ui_elements_classification = models.ForeignKey(
        "UIElementsClassification", on_delete=models.SET_NULL, null=True
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "featureextraction:ui_detection_list", args=[str(self.case_study_id)]
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.type == "screen2som" and not self.ui_elements_classification:
            ui_elements_classification = UIElementsClassification.objects.create(
                ...
                model=CNNModels.objects.get_or_create(
                    name="screen2som",
                    path="NA",
                    image_shape=[640, 360, 3],
                    classes=get_ui_elements_classification_screen2som(),
                    text_classname="Text",
                )[0],
                type=self.type,
                skip=self.skip,
...
def get_ui_elements_classification_moran():
    return "x0_Button,x0_CheckBox,x0_CheckedTextView,x0_EditText,x0_ImageButton,x0_ImageView,x0_NumberPicker,x0_RadioButton,x0_RatingBar,x0_SeekBar,x0_Spinner,x0_Switch,x0_TextView,x0_ToggleButton".split(
        ","
    )  # this returns a list


def get_ui_elements_classification_uied():
    return "Button,Checkbox,CheckedTextView,EditText,ImageButton,ImageView,NumberPicker,RadioButton,RatingBar,SeekBar,Spinner,Switch,TextView,ToggleButton".split(
        ","
    )  # this returns a list
...
class CNNModels(models.Model):
    ...
    image_shape = ArrayField(
        models.IntegerField(blank=True),
        default=get_ui_elements_classification_image_shape,
    )
    classes = ArrayField(
        models.CharField(max_length=50), default=get_ui_elements_classification_uied
    )
    ...
    def clean(self):
        if self.text_classname not in self.classes:
            raise ValidationError(
                "text_classname must be one of the ui_elements_classification_classes"
            )
...
class UIElementsClassification(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "featureextraction:ui_classification_list", args=[str(self.case_study_id)]
        )
...
class Postfilters(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "featureextraction:postfilters_list", args=[str(self.case_study_id)]
        )
...
class FeatureExtractionTechnique(models.Model):
    ...
    identifier = models.CharField(
        ...
        validators=[
            RegexValidator(
                r"^[a-zA-Z-]*$",
                _("Only lowercase, uppercase letters, and hyphen are allowed"),
            )
        ],
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "featureextraction:fe_technique_list", args=[str(self.case_study_id)]
        )
...
class Postprocessing(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    technique_name = models.CharField(max_length=255, null=True)
    # How to combine gaze data when multiple gaze sources are available
    gaze_conciliation = models.CharField(
        max_length=20,
        choices=GAZE_CONCILIATION_CHOICES,
        default="union",
        null=False,
        blank=False,
    )
    ...
    def get_absolute_url(self):
        return reverse(
            "featureextraction:postprocessing_list", args=[str(self.case_study_id)]
        )
