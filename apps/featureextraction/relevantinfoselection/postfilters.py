...
from core.settings import PLATFORM_NAME, INFO_POSTFILTERING_PHASE_NAME
from apps.featureextraction.utils import draw_geometry_over_image
# translation helper removed because not used in this module


def calculate_intersection_area_v1(
...
def is_component_relevant_v1(compo, fixation_point, fixation_point_x, fixation_point_y):
    ...
    # (previously calculated but unused) fixation area intentionally omitted
    ...
    rect_area = (intersection_x2 - intersection_x1) * (
        intersection_y2 - intersection_y1
    )
    circle_area = (
        math.pi * min(dispersion, (intersection_x2 - intersection_x1) / 2) ** 2
    )
...
def calculate_intersection_area_v2(
    compo, fixation_point_x, fixation_point_y, circle_radius
):
    ...
    if (
        fixation_point_x >= x_min
        and fixation_point_x <= x_max
        and fixation_point_y >= y_min
        and fixation_point_y <= y_max
    ):
        intersection_area = circle_radius**2
    else:
        distance = (
            (fixation_point_x - closest_x) ** 2 + (fixation_point_y - closest_y) ** 2
        ) ** 0.5
        if distance < circle_radius:
            intersection_area = (
                circle_radius**2 * math.acos(distance / circle_radius)
...
def build_fixation_mask(fixation_json, screenshot_filename):
    """
    Build a Shapely geometry that is the union of fixation circles for a given screenshot.

    Returns a geometry (possibly an empty GeometryCollection) that can be used
    to compute intersections with component polygons.
    """
    circles = []
    # Safely get fixation points dict for the screenshot
    fixation_points = fixation_json.get(screenshot_filename, {}).get(
        "fixation_points", {}
    )
    for fixation_key, fixation_obj in fixation_points.items():
        # Ensure dispersion is present
        if "dispersion" not in fixation_obj:
            continue
        # Parse coordinates from the fixation key (format: "x#y")
        try:
            coord_parts = fixation_key.split("#", 1)
            fixation_point_x = float(coord_parts[0])
            fixation_point_y = float(coord_parts[1])
        except Exception:
            # Skip malformed fixation points
            continue

        try:
            radius = float(fixation_obj["dispersion"])
        except Exception:
            continue
        ...
        circles.append(centre.buffer(radius))

    # Return the union of all fixation circles (returns an empty geometry if none)
...
def gaze_filtering(
    log_path, path_scenario, special_colnames, consider_nested_as_relevant
):
    """
    ...
    for screenshot_filename in ui_log[special_colnames["Screenshot"]]:
        ...
        if (
            screenshot_filename not in fixation_json.keys()
            or fixation_json[screenshot_filename] == {}
        ):
            continue
        ...
        with open(
            os.path.join(
                scenario_results_path, "components_json", screenshot_filename + ".json"
            ),
            "r",
        ) as f:
            screenshot_json = json.load(f)

        # Build fixation mask as the union of circles around fixation points
        fixation_mask = build_fixation_mask(fixation_json, screenshot_filename)

        # Iterate over each component in the screenshot.json file
        polygon_rectangles = filter_relevant_ui_components(
            consider_nested_as_relevant,
            fixation_json,
            screenshot_filename,
            screenshot_json["compos"],
            fixation_mask,
        )

        if not os.path.exists(
            os.path.join(scenario_results_path, "postfilter_attention_maps")
        ):
            os.makedirs(
                os.path.join(scenario_results_path, "postfilter_attention_maps")
            )
        ...
        draw_geometry_over_image(
            os.path.join(
                scenario_results_path, "borders", screenshot_filename + "_bordered.png"
            ),
            fixation_mask,
            polygon_rectangles,
            os.path.join(
                scenario_results_path, "postfilter_attention_maps", screenshot_filename
            ),
        )
        ...
        # Save fixation mask as an npy
        fixation_array: np.array
        if isinstance(fixation_mask, (MultiPolygon, GeometryCollection)):
            fixation_array = np.array(fixation_mask.geoms)
        elif isinstance(fixation_mask, BaseGeometry):
            fixation_array = np.array([fixation_mask])
        else:
            fixation_array = fixation_mask
        np.save(
            os.path.join(
                scenario_results_path,
                "postfilter_attention_maps",
                screenshot_filename[:-4] + ".npy",
            ),
            fixation_array,
        )
...
def filter_relevant_ui_components(
    consider_nested_as_relevant,
    fixation_json,
    screenshot_filename,
    compos,
    ...
    polygon_rectangles = []
    for compo in compos:
        # Initialize the "relevant" field of the component as False
    ...
    for compo_json_filename in arr:
        ...
        for compo in compo_json["compos"]:
            if compo.get("relevant"):
                ...
                cv2.rectangle(
                    image, (row_min, column_min), (row_max, column_max), border_color, 2
                )

        # Save the image with component borders
        output_path = os.path.join(
            exp_path + " _results", "compo_json_borders", compo_json_filename[:19]
