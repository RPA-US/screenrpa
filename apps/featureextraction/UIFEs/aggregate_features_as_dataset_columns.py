...
def occurrence_ui_element_class(ui_log_path, path_scenario, execution, fe):
    ...
    # Create the flattened log for each decision point, which we read from the existing flattened logs
    flattened_logs = [
        os.path.join(path_scenario_results, f)
        for f in os.listdir(path_scenario_results)
        if f.startswith("flattened_dataset_") and f.endswith(".json")
    ]
    ...
    for flattened_log in flattened_logs:
        ...
        dp_activity = re.search(
            r"(?<=flattened_dataset_)\d+(?=[.-])", flattened_log
        ).group(0)

        cases = list(ui_log_data.keys())

        for case in cases:
            case_log = log[log[case_colname] == int(case)][
                log[activity_colname] <= int(dp_activity)
            ].reset_index(drop=True)

            screenshot_filenames = case_log.loc[
                :, screenshot_colname
            ].values.tolist()  # Screenshots of the case
            for i, screenshot_filename in enumerate(screenshot_filenames):
                activity = case_log.at[i, activity_colname]
                ...
                # Count UI element classes
                class_counter = Counter(compo["class"] for compo in compos_list)

                # Handle status values separately
                for compo_class in ui_elements_classification_classes:
                    if "_" in compo_class:  # For status values
                        st = compo_class.split("_")
                        class_name, status_value = st[0], st[1]
                        st_key = find_st_id(status_value)

                        # Count elements with matching class and status
                        count = sum(
                            1
                            for compo in compos_list
                            if compo["class"] == class_name
                            and st_key in compo
                            and compo[st_key] == status_value
                        )

                        # quantity_ui_elements[compo_class] += count
                        ui_log_data[str(case)][
                            id + "_" + compo_class + "_" + str(activity)
                        ] = count
                    else:
                        # Use the counts from class_counter
                        count = class_counter[compo_class]
                        # quantity_ui_elements[compo_class] += count
                        ui_log_data[str(case)][
                            id + "_" + compo_class + "_" + str(activity)
                        ] = count
        ...
        # save csv equivalent
        pd.DataFrame.from_dict(ui_log_data, orient="index").to_csv(
            flattened_log.replace(".json", ".csv")
        )
...
def state_ui_element_centroid(ui_log_path, path_scenario, execution, fe):
    ...
    log = read_ui_log_as_dataframe(
        os.path.join(path_scenario + "_results", PROCESS_DISCOVERY_LOG_FILENAME)
    )
    ...
    flattened_logs = [
        os.path.join(execution_root, f)
        for f in os.listdir(execution_root)
        if f.startswith("flattened_dataset") and f.endswith(".json")
    ]
        ...
        for i, screenshot_filename in enumerate(screenshot_filenames):
            ...
            if before_DP:
                ...
                for j in range(0, len(compos_list)):
                    ...
                    # Status columns
                    status_columns = [
                        i for i in dict(compos_list[j]).keys() if "st_" in i
                    ]
