...
def scene_level(log_path, scenario_path, execution):
    ...
    def extract_image_text(row, image_col):
        image_som = json.load(
            open(
                os.path.join(
        ...
        text = " ".join(
            list(
                map(
                    lambda c: c["text"],
                    filter(
                        lambda c: c["class"] == "Text", image_som["som"]["children"]
                    ),
                )
            )
        )
    ...
    def extract_features_from_images(
        df, scenario_path, image_col, text_col, image_weight, text_weight, model_type
    ):
        ...
        if model_type == "clip":
            pbar = tqdm(
                total=df.shape[0], desc="Extracting features with CLIP from Image"
            )
        ...
        for index, row in df.iterrows():
            ...
            if model_type == "clip":
                text = (
                    row[text_col]
                    if text_col != "Top Level Text"
                    else extract_image_text(row, image_col)
                )
                features = process_image_clip(
                    image,
                    text,
    ...
    def perform_clustering(features, method, n_clusters=2):
        if method == "agglomerative":
            ...
            raise ValueError(
                "Unsupported clustering method. Choose 'agglomerative' or 'divisive'."
            )
    ...
    def find_optimal_clusters(
        features, clustering_type, min_clusters=2, max_clusters=10
    ):
        best_score = -1
        optimal_clusters = min_clusters

        for k in tqdm(
            range(min_clusters, max_clusters + 1), desc="Finding optimal clusters"
        ):
        ...
        optimal_labels = perform_clustering(
            features, clustering_type, n_clusters=optimal_clusters
        )
        silhouette_avg, davies_bouldin, calinski_harabasz = evaluate_clusters(
            features, optimal_labels
        )

        tqdm.write(
            f"Method: {clustering_type}, Optimal Clusters: {optimal_clusters}, Best Silhouette Score: {silhouette_avg}"
        )
        tqdm.write(
            f"Davies-Bouldin Score: {davies_bouldin}, Calinski-Harabasz Score: {calinski_harabasz}"
        )
    ...
    def auto_labeling(df, fe_log, remove_loops):
        if remove_loops:
            df, fe_log = remove_duplicate_activities(
                df, fe_log, special_colnames["Activity"]
            )
    ...
    def trace_id_assignment(df, fe_log, remove_loops, labeling_mode="automatic"):
        if labeling_mode == "automatic":
            ...
            raise ValueError(
                "Unsupported labeling mode. Choose 'automatic' or 'manual'."
            )
        return df, fe_log
    ...
    def generate_dendrogram(
        df, show_dendrogram, features_column="combined_features", folder_path="results"
    ):
    ...
    if execution.feature_extraction_technique:
        fe_log = read_ui_log_as_dataframe(
            os.path.join(
                scenario_path + "_results", "log" + ENRICHED_LOG_SUFFIX + ".csv"
            )
        )
    else:
        fe_log = None
    ui_log = extract_features_from_images(
        ui_log,
        scenario_path,
    ...
    ui_log.to_csv(
        os.path.join(folder_path, PROCESS_DISCOVERY_LOG_FILENAME), index=False
    )
...
def process_level(folder_path, df, fe_log, execution):
    ...
    def petri_net_process(df, special_colnames):
        formatted_df = pm4py.format_dataframe(
            df,
            case_id=special_colnames["Case"],
            activity_key=special_colnames["Activity"],
            timestamp_key=special_colnames["Timestamp"],
        )
    ...
    def bpmn_process(df, fe_log, special_colnames):
        ...
        node_start = list(
            filter(
                lambda node: type(node) == pm4py.objects.bpmn.obj.BPMN.StartEvent, nodes
            )
        )[0]
        node_end = list(
            filter(
                lambda node: type(node) == pm4py.objects.bpmn.obj.BPMN.NormalEndEvent,
                nodes,
            )
        )[0]

        def explore_branch(node_start, visited, last_act) -> tuple[
            Branch,
            pm4py.objects.bpmn.obj.BPMN.ExclusiveGateway
            | pm4py.objects.bpmn.obj.BPMN.NormalEndEvent,
            set,
        ]:
            ...
            while cn != node_end:
                ...
                if iteration_count > max_iterations:
                    raise Exception(
                        "Infinite loop detected in BPMN exploration. Exceeded maximum iterations."
                    )

                # If the next node is a converging ExclusiveGateway or a NormalEndEvent, the branch is finished
                if (
                    type(cn) == pm4py.objects.bpmn.obj.BPMN.ExclusiveGateway
                    and cn.get_gateway_direction()
                    == pm4py.objects.bpmn.obj.BPMN.ExclusiveGateway.Direction.CONVERGING
                    or type(cn) == pm4py.objects.bpmn.obj.BPMN.NormalEndEvent
                ):
                ...
                if cn in visited:
                    raise Exception(
                        "error: end node not reached. current bpmn_bfs does not support loops. state:"
                        + cn.name
                        + " "
                        + cn.id
                        + " "
                ...
                if (
                    type(cn) == pm4py.objects.bpmn.obj.BPMN.Task
                    or type(cn) == pm4py.objects.bpmn.obj.BPMN.StartEvent
                ):
                    visited.add(cn)
                    last_act = cn.name
                    cn = next_node
                ...
                # A DecisionPoint is created and added to the dps list
                    ...
                    type(cn) == pm4py.objects.bpmn.obj.BPMN.ExclusiveGateway
                    and cn.get_gateway_direction()
                    == pm4py.objects.bpmn.obj.BPMN.ExclusiveGateway.Direction.DIVERGING
                ):
                    visited.add(cn)
                    ...
                    for arc in cn.get_out_arcs():
                        ...
                        if (
                            type(arc.target)
                            == pm4py.objects.bpmn.obj.BPMN.ExclusiveGateway
                            and arc.target.get_gateway_direction()
                            == pm4py.objects.bpmn.obj.BPMN.ExclusiveGateway.Direction.CONVERGING
                        ):
                            ...
                            branch, converge_gateway, visited = explore_branch(
                                arc.target, visited, last_act
                            )
                            rule = Rule([], arc.target.name)
                        rules.append(rule)
                    ...
                    if (
                        ...
                        return (
                            Branch(branch_start.name, branch_start.id, dps),
                            next_node,
                            visited,
                        )
                    else:
                ...
                # Handling any other case
                    ...
                    raise Exception(
                        f"ScreenRPA does not currently support with processes containing elements of type {type(cn)}"
                    )
        ...
        def bpmn_bfs(node_start, node_end) -> Process:
            return Process(
                explore_branch(node_start, set(), node_start.name)[0].decision_points
            )
        ...
        def add_trace_to_log(process, df) -> None:
            ...
            for index, row in df.iterrows():
                if str(row[special_colnames["Case"]]) != str(current_trace_id):
                    ...
                    current_trace_rows = df.loc[
                        df[special_colnames["Case"]] == current_trace_id
                    ]
                ...
                if current_dp is not None:
                    if any(str(branch.label) == str(act_label) for branch in branches):
                        ...
                        branch_label = list(
                            filter(
                                lambda branch: str(branch.label) == str(act_label),
                                branches,
                            )
                        )[0].label
        ...
        try:
            ...
            json.dump(
                process.to_json(),
                open(os.path.join(folder_path, "traceability.json"), "w"),
            )
...
class ProcessDiscoveryListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = ProcessDiscovery.objects.filter(
...
class ProcessDiscoveryDetailView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        return get_object_or_404(
            ProcessDiscovery, id=self.kwargs.get("process_discovery_id")
        )
    ...
    def get(self, request, *args, **kwargs):
        process_discovery = get_object_or_404(
            ProcessDiscovery, id=kwargs["process_discovery_id"]
        )
        if process_discovery.case_study.user != request.user:
            raise PermissionDenied(
                "Process Discovery doesn't belong to the authenticated user."
            )

        process_discovery_form = ProcessDiscoveryForm(
            read_only=process_discovery.freeze, instance=process_discovery
        )
...
def set_as_process_discovery_active(request):
    ...
    elif not ProcessDiscovery.objects.get(
        pk=process_discovery_id
    ).case_study == CaseStudy.objects.get(pk=case_study_id):
        raise PermissionDenied("Case Study doesn't belong to the authenticated user.")
    process_discovery_list = ProcessDiscovery.objects.filter(
    ...
    return HttpResponseRedirect(
        reverse("processdiscovery:processdiscovery_list", args=[case_study_id])
    )
...
def set_as_process_discovery_inactive(request):
    ...
    if ProcessDiscovery.objects.get(
        pk=process_discovery_id
    ).case_study != CaseStudy.objects.get(pk=case_study_id):
...
def dot_to_png_base64(dot_path):
    try:
        ...
        with NamedTemporaryFile(delete=False, suffix=".png") as temp_file:
            graph_path = graph.render(
                filename=temp_file.name, format="png", cleanup=True
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if scenario is None:
            scenario = execution.scenarios_to_study[
                0
            ]  # por defecto, el primer escenario indicado

        path_to_bpmn_file = os.path.join(
            execution.exp_folder_complete_path, scenario + "_results", "bpmn.dot"
        )

        df = read_ui_log_as_dataframe(
            os.path.join(
        ...
        if selected_variant:
            labels_for_variant = (
                df[df[colnames["Variant"]] == int(selected_variant)][
                    execution.case_study.special_colnames["Activity"]
                ]
                .unique()
                .tolist()
            )
            modified_dot_path = cambiar_color_nodos_y_caminos(
                labels_for_variant, path_to_bpmn_file
            )
...
def ProcessDiscoveryDownload(request, execution_id):
    ...
    path_to_bpmn_dot = os.path.join(
        execution.exp_folder_complete_path, scenario + "_results", "bpmn.dot"
    )
    path_to_bpmn_bpmn = os.path.join(
        execution.exp_folder_complete_path, scenario + "_results", "bpmn.bpmn"
    )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zip_file:
            for file_path, arcname in [
                (path_to_bpmn_dot, "bpmn.dot"),
                (path_to_bpmn_bpmn, "bpmn.bpmn"),
            ]:
        ...
        return HttpResponse(
            "Sorry, there was an error processing your request.", status=500
        )
