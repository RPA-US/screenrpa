...
def flat_dataset_row(
    ...
    try:
        json_traceability = json.load(
            open(os.path.join(path_dataset_saved, "traceability.json"))
        )
    ...
    if not activities_before_dps or len(activities_before_dps) == 0:
        raise ValueError(
            "The activities_before_dps list is empty. Please, provide a valid list of activities before the decision point or check the process model discovered."
        )
    ...
    for i, (act, dp) in enumerate(activities_before_dps):
        ...
        for index, c in enumerate(cases):
            ...
            if c != last_case:
                if cases[index - 1] in log_dict.keys():
                    log_dict[cases[index - 1]]["Timestamp_end"] = log.at[
                        index - 1, timestamp_column_name
                    ]
                before_DP = True
            ...
            if before_DP:
                ...
                for feat in columns:
                    if (
                        re.match(
                            r"id[a-zA-Z0-9]{8}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]+",
                            feat,
                        )
                        and feat not in current_post_dps
                        and type(log.at[index, feat]) == str
                    ):
                        log_dict[c][feat] = log.at[index, feat]
                    elif feat not in actions_columns and not re.match(
                        r"id[a-zA-Z0-9]{8}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]+",
                        feat,
                    ):
            ...
            if str(activity) == act:
                ...
                if str(branch) != "nan":
                    try:
                        log_dict[c]["dp_branch"] = str(int(float(branch)))
                    except Exception:
                        log_dict[c]["dp_branch"] = str(branch)
                else:
                    del log_dict[c]
                    continue
        ...
        if cases[len(cases) - 1] in log_dict.keys():
            log_dict[cases[len(cases) - 1]]["Timestamp_end"] = log.at[
                len(cases) - 1, timestamp_column_name
            ]
        ...
        if os.path.exists(
            os.path.join(path_dataset_saved, f"flattened_dataset_{act}.csv")
        ):
            i = 1
            while True:
                if not os.path.exists(
                    os.path.join(path_dataset_saved, f"flattened_dataset_{act}-{i}.csv")
                ):
                    aux_path = os.path.join(
                        path_dataset_saved, f"flattened_dataset_{act}-{i}"
                    )
